...
class BysykkelController:
    ...
    def report_bike_issues(self, bike_id, issues, notes=None):
        ...
        return self.model.report_bike_issue(bike_id, issues)
    ...
    def get_stations_availability(self, in_progress=False):
        ...
        stations_df['Location'] = stations_df.apply(
            lambda row: f'<a href="https://www.openstreetmap.org/#map=17/{row.Latitude}/{row.Longitude}">Map</a>', 
            axis=1
        )
        ...
        result_df['Availability'] = result_df['Availability'].astype(str) + '%'
