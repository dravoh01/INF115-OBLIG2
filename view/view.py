...
class BysykkelView:
    ...
    def show_dropoff_tab(self, tab, users_with_active_trips, stations_df):
        """Display the simplified dropoff interface with integrated issue reporting"""
        with tab:
            ...
            # Step 1: Select user and automatically show their active trip
            if st.session_state.dropoff_step == "select_user":
                st.subheader("Select User with Active Trip")
                
                if users_with_active_trips.empty:
                    st.warning("No users currently have active trips")
                    return {
                ...
                # Create user options with active trip info
                user_options = users_with_active_trips['User_ID'].unique().tolist()
                
                if user_options:
                    selected_user = st.selectbox(
                        "Select a user:",
                        options=user_options,
                        format_func=lambda x: f"{users_with_active_trips[users_with_active_trips['User_ID'] == x]['User_Name'].values[0]} ({x})",
                        key="dropoff_user"
                    )
                    
                    # Get the trip details for this user
                    user_trip = users_with_active_trips[users_with_active_trips['User_ID'] == selected_user]
                    
                    if not user_trip.empty:
                        # There should be only one active trip per user
                        trip_row = user_trip.iloc[0]
                        
                        # Display trip information
                        st.info(f"""
                        **Active Trip Details:**
                        - Trip ID: {trip_row['Trip_ID']}
                        - Bike: {trip_row['Bike_Name']} (ID: {trip_row['Bike_ID']})
                        - Started from: {trip_row['Start_Station_Name']}
                        - Start time: {trip_row['Start_Time']}
                        """)
                        
                        # Get bike ID and trip ID
                        bike_id = trip_row['Bike_ID']
                        ...
                        if not stations_df.empty:
                            ...
                            if dropoff_button:
                                ...
                                st.session_state.dropoff_bike_name = trip_row['Bike_Name']
                                st.session_state.dropoff_station_id = selected_station
                                st.session_state.dropoff_trip_id = trip_id
                            ...
                            return {
                                ...
                                "user_id": selected_user,
                                "bike_id": bike_id, 
                                "station_id": selected_station if selected_station else None,
                                "trip_id": trip_id
                            }
                        else:
                            st.warning("No stations available")
                    ...
                    st.warning("No users with active trips")
                    return {
                        "dropoff_button": False,
                        "user_id": None,
                        "bike_id": None,
        ...
        if dropoff_button:
            ...
            # Debug information
            print(f"Dropoff button clicked with:")
            print(f"- User ID: {selected_user}")
            print(f"- Bike ID: {bike_id} (type: {type(bike_id)})")
    ...
    def show_mapping_tab(self, tab, stations_df):
        ...
        with tab:
            ...
            with col2:
                ...
                # Button to refresh the view with the current toggle state
                refresh = st.button("Refresh", key="refresh_mapping")
        
            # Display the table with availability info
            if selected_station is not None:
                # Filter the data for the selected station in the controller
